"""SQLZOO: Window functions Solutions"""
...
Include year, party, votes, and ranking (the party with the most votes is 1)."
...
FROM (SELECT constituency, party, SUM(votes), RANK() OVER
   (PARTITION BY constituency ORDER BY SUM(votes) DESC) as rank
   FROM ge
   WHERE constituency BETWEEN 'S14000021' AND 'S14000026'
   AND yr = 2017
   GROUP BY constituency, party) as x
WHERE x.rank = 1
...
WHERE constituency LIKE 'S%' AND yr=2017
   AND votes = (SELECT MAX(votes) FROM ge y
